...
func (rex *RoundExecutor) executeTransactionPart1FabricAndFabricCRDT(counter int, startTime time.Time) {
	transactionResult := MakeNewTransactionResultFabricAndFabricCRDT(counter, rex.endorsementPolicyOrgs, rex.signer)
	...
	transactionResult.latencyMeasurementInstance = transactionResult.latencyMeasurement(startTime) //transactionResult.latencyMeasurement(time.Now())
...
func (rex *RoundExecutor) streamTransactionFabricAndFabricCRDT(transactionResult *TransactionResult, transaction *protos.Transaction) {
	...
	for orderer := range rex.executor.orderersConnections {
		...
		if err := streamer.streamFabricAndFabricCRDT.Send(transaction); err != nil {
			...
			err = rex.executor.makeSingleStreamTransactionFabricAndFabricCRDT(orderer)
			if err != nil {
				logger.ErrorLogger.Println(err)
			}
		}
...
func (ex *Executor) subscriberForTransactionEventsFabricAndFabricCRDT() {
...
func (ex *Executor) makeAllStreamTransactionFabricAndFabricCRDT() {
	for orderer := range ex.orderersConnections {
		if err := ex.makeSingleStreamTransactionFabricAndFabricCRDT(orderer); err != nil {
			logger.ErrorLogger.Println(err)
		}
	}
}

func (ex *Executor) makeSingleStreamTransactionFabricAndFabricCRDT(orderer string) error {
	...
	if err != nil {
		...
		connpool.SleepAndReconnect()
		err = ex.makeSingleStreamTransactionFabricAndFabricCRDT(orderer)
		if err != nil {
			logger.ErrorLogger.Println(err)
		}
		return nil
