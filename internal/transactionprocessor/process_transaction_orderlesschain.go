...
func (p *Processor) addTransactionToDatabaseOrderlessChain(tx *protos.Transaction) error {
	...
	var err error
	keyValueLength := len(tx.ReadWriteSet.WriteKeyValues.WriteKeyValues)
	if keyValueLength == 1 {
		...
		wg := &sync.WaitGroup{}
		wg.Add(keyValueLength)
		for _, keyValue := range tx.ReadWriteSet.WriteKeyValues.WriteKeyValues {
			go p.addKeyValueToDatabaseWithWaitGroupOrderlessChain(keyValue, tx, dbOp, &err, wg)
		}
		wg.Wait()
	}
	return err
