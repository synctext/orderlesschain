...
// source: https://github.com/processout/grpc-go-pool
...
// FactoryWithIP is a function type creating a grpc client with specific cIP
type FactoryWithIP func(string) (*grpc.ClientConn, error)
...
// NewPool creates a new clients pool with the given initial and maximum capacity,
// and the timeout for the idle clients. Returns an error if the initial
// clients could not be created
func NewPool(factory Factory, init, capacity int, idleTimeout time.Duration,
	maxLifeDuration ...time.Duration) (*Pool, error) {
	return NewPoolWithContext(context.Background(), func(ctx context.Context) (*grpc.ClientConn, error) { return factory() },
		init, capacity, idleTimeout, maxLifeDuration...)
}

// NewPoolWithIP creates a new clients pool with the given initial and maximum capacity,
// and the timeout for the idle clients. Returns an error if the initial
// clients could not be created
func NewPoolWithIP(factory FactoryWithIP, ip string, init, capacity int, idleTimeout time.Duration,
	maxLifeDuration ...time.Duration) (*Pool, error) {
	return NewPoolWithContext(context.Background(), func(ctx context.Context) (*grpc.ClientConn, error) { return factory(ip) },
		init, capacity, idleTimeout, maxLifeDuration...)
}

// NewPoolWithContext creates a new clients pool with the given initial and maximum
...
func NewPoolWithContext(ctx context.Context, factory FactoryWithContext, init, capacity int, idleTimeout time.Duration,
	maxLifeDuration ...time.Duration) (*Pool, error) {
