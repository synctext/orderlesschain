...
runAuctionOrderlessChain(){
    ...
    for BENCHMARK in  auction.orderlesschain1 auction.orderlesschain2 auction.orderlesschain3 auction.orderlesschain4 auction.orderlesschain5; do
        for i in {1..1}; do
          echo "Benchmark $BENCHMARK is executing for $i times"
          go run ./cmd/client -coordinator=true -benchmark=${BENCHMARK}
          commitResults
...
runVoteOrderlessChain(){
    ...
    for BENCHMARK in vote.orderlesschain1 vote.orderlesschain2 vote.orderlesschain3 vote.orderlesschain4 vote.orderlesschain5; do
        for i in {1..1}; do
          echo "Benchmark $BENCHMARK is executing for $i times"
          go run ./cmd/client -coordinator=true -benchmark=${BENCHMARK}
          commitResults
...
runAuctionFabric(){
    ...
    for BENCHMARK in auction.fabric1 auction.fabric2 auction.fabric3  auction.fabric4 auction.fabric5; do
        for i in {1..1}; do
          echo "Benchmark $BENCHMARK is executing for $i times"
          go run ./cmd/client -coordinator=true -benchmark=${BENCHMARK}
          commitResults
...
runVoteFabric(){
    ...
    for BENCHMARK in vote.fabric1 vote.fabric2  vote.fabric3 vote.fabric4 vote.fabric5; do
        for i in {1..1}; do
          echo "Benchmark $BENCHMARK is executing for $i times"
          go run ./cmd/client -coordinator=true -benchmark=${BENCHMARK}
          commitResults
...
runAuctionFabricCRDT(){
    ...
    for BENCHMARK in  auction.fabriccrdt1 auction.fabriccrdt2 auction.fabriccrdt3 auction.fabriccrdt4 auction.fabriccrdt5; do
        for i in {1..1}; do
          echo "Benchmark $BENCHMARK is executing for $i times"
          go run ./cmd/client -coordinator=true -benchmark=${BENCHMARK}
          commitResults
...
runVoteFabricCRDT(){
    ...
    for BENCHMARK in vote.fabriccrdt1  vote.fabriccrdt2  vote.fabriccrdt3   vote.fabriccrdt4   vote.fabriccrdt5; do
        for i in {1..1}; do
          echo "Benchmark $BENCHMARK is executing for $i times"
          go run ./cmd/client -coordinator=true -benchmark=${BENCHMARK}
          commitResults
